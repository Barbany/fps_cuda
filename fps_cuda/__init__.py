...
""" FPS cuda has to be imported after torch as it requires libc10.so, loaded by torch """
try:
...
def fps(data: torch.Tensor, number: int) -> torch.Tensor:
    ...
    Args:
        data: Torch tensor with shape (batch_size, num_points, num_features)
        number: Number of points to sample for each sample in the batch.

    Returns:
        Torch tensor with shape (batch_size, number, num_features)
    """
    fps_idx = furthest_point_sample(data, number)
    fps_data = (
        gather_operation(data.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()
    )
    return fps_data
...
class FurthestPointSampling(Function):
    ...
    def forward(ctx, xyz, npoint):
        ...
        r"""
        Uses iterative furthest point sampling to select a set of npoint features that have
